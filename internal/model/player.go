...
// 计算公式：基础值(100 + 等级*20) + 体质*10 + 力量*5 + 防御*3
func (p *Player) CalcMaxHp() int64 {
	base := int64(100 + p.Level*20)
	return base + int64(p.Con)*10 + int64(p.Str)*5 + int64(p.Def)*3
}

// CalcAttack 计算玩家攻击力
// 公式：力量*2 + 等级*5 + 敏捷*0.5
func (p *Player) CalcAttack() int64 {
	return int64(p.Str)*2 + int64(p.Level)*5 + int64(float64(p.Agi)*0.5)
}

// CalcDefense 计算玩家防御力
// 公式：防御属性*3 + 体质*1 + 等级*2
func (p *Player) CalcDefense() int64 {
	return int64(p.Def)*3 + int64(p.Con) + int64(p.Level)*2
}
